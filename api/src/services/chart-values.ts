...
export const historicalPriceLookups = async ({
  db,
  assetIds,
  quoteCurrencies,
  fromMs,
  ...
  const byAssetAndCurrency = Map.groupBy(
    rows,
    (row) => `${row.canonical_asset_id}:${row.quote_currency.toUpperCase()}`
  );
  const cache = new Map<string, string | null>();

  return new Map(normalizedCurrencies.map((currency) => [
    currency,
    ({ assetId, timestampMs }: { assetId: string; timestampMs: number }) => {
      if (assetId === currency.toLowerCase()) return '1';
      const cacheKey = `${assetId}:${currency}:${timestampMs}`;
      if (cache.has(cacheKey)) return cache.get(cacheKey) ?? null;
      const candidates = byAssetAndCurrency.get(`${assetId}:${currency}`) ?? [];
      let left = 0;
...
export const historicalPriceLookup = async ({
  quoteCurrency,
  ...input
}: Omit<Parameters<typeof historicalPriceLookups>[0], 'quoteCurrencies'> & {
  quoteCurrency: string;
}) => {
