  ...
  async reconcileAll() {
    ...
    for (const ledger of ledgers) {
      ...
      const candidates = chainEvents
        .filter((event) => event.canonical_asset_id === ledger.canonical_asset_id)
        .map((event) => ({
          event,
        ...
        .sort((left, right) => {
          ...
          return rank[left.match.confidence] - rank[right.match.confidence];
        });
      ...
      await this.db.run({
        sql: `
          ...
          ON CONFLICT(kraken_ledger_id, chain_transaction_id, address_balance_event_id)
          DO UPDATE SET
